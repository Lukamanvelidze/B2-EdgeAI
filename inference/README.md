...
| Script                          | Description |
|---------------------------------|-------------|
| `inference.py`                  | Performs live inference using only the YOLOv8n object detection model. Annotates frames with bounding boxes and displays them in real time. |
| `2stage_inference.py`           | Implements a two-stage inference pipeline: detects objects using YOLOv8n, then classifies each detection using a trained classifier model. |
